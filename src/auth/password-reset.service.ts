...
export class PasswordResetService {
  ...
  async requestPasswordReset(email: string): Promise<{ message: string }> {
    ...
    // ✅ CORRIGIDO: Usando sendPasswordReset (conforme MailService)
  ...
  async validateResetToken(token: string): Promise<{ valid: boolean; email?: string }> {
    ...
    if (!resetRequest) return { valid: false };
    if (resetRequest.used) return { valid: false };
    if (new Date() > resetRequest.expiresAt) return { valid: false };
  ...
  async resetPassword(token: string, newPassword: string): Promise<{ message: string }> {
    ...
    if (!resetRequest) throw new BadRequestException('Token inválido ou expirado.');
    if (resetRequest.used) throw new BadRequestException('Este link já foi utilizado.');
    if (new Date() > resetRequest.expiresAt) throw new BadRequestException('Este link expirou. Solicite um novo.');
    ...
    // ✅ CORRIGIDO: Usando sendPasswordChanged (conforme MailService)
